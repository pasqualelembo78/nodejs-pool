...
function Coin(data){
    ...
    this.getPortAnyBlockHeaderByHash = function(port, blockHash, is_our_block, callback){
        ...
        if (port == 11898) {
            ...
            global.support.rpcPortDaemon(port, 'GetHeaderByHash', {"hash": Buffer.from(blockHash, 'hex').toJSON().data}, function (body) {
                ...
                return callback(null, body.result.header);
            });
    ...
    this.getPortLastBlockHeader = function(port, callback, no_error_report) {
		...
		return callback(null, { hash: bt.hash, timestamp: Date.now() / 1000, difficulty: bt.difficulty, height: bt.height, hash2: bt.hash2 });
            ...
            global.support.rpcPortDaemon(port, 'GetTipInfo', null, function(body) {
                if (typeof(body) !== 'undefined' && body.hasOwnProperty('result')) {
                    global.support.rpcPortDaemon(port, 'GetNewBlockTemplate', {"algo":{"pow_algo":0}}, function(is_err, body2) {
                        if (!is_err && typeof(body2) !== 'undefined' && body2.hasOwnProperty('result')) {
                            body.result.metadata.reward = body2.result.miner_data.reward;
                            body.result.metadata.difficulty = body2.result.miner_data.target_difficulty;
                            return callback(false, body.result.metadata);
                        } else {
                            if (!no_error_report) console.error("New block template invalid: " + JSON.stringify(body2));
                            return callback(true, body2);
                        }
                    });
                    global.coinFuncs.getPortAnyBlockHeaderByHash(port, body.result.metadata.bestBlockHash, false, function(is_err,  body2) {
                        return callback(is_err, body2);
                    });
                } else {
                    if (!no_error_report) console.error("Last block header invalid: " + JSON.stringify(body));
                    return callback(true, body);
    ...
    this.getPortBlockTemplate = function(port, callback) {
		...
		method: 'getblocktemplate',
           ...
           global.support.rpcPortDaemon(port, 'GetNewBlockTemplate', { algo: {pow_algo:0 } }, function(body){
                if (body && body.result) {
                   body.result.newBlockTemplate.difficulty = body.result.minerData.targetDifficulty;
                   body.result.newBlockTemplate.reward = body.result.minerData.reward;
                   return callback(body.result.newBlockTemplate);
                } else return callback(null);
            });
    ...
    this.constructNewBlob = function(blockTemplate, params, port) {
        ...
        if (global.coinFuncs.blobTypeGrin(blob_type_num)) {
            ...
            return cnUtil.constructNewKcnBlob(blockTemplate, Buffer.from(params.nonce, 'hex'));
        } else {
    ...
    this.BlockTemplate = function(template) {
        ...
        this.wide_difficulty    = template.wide_difficulty ? parseInt(template.wide_difficulty, 16) : this.difficulty;
        const aux_chain_txm     = global.coinFuncs.getAuxChainXTM(template);
        this.txm_height         = aux_chain_txm ? aux_chain_txm.height : 0;
        ...
        if (global.coinFuncs.blobTypeRvn(port_blob_num) || global.coinFuncs.blobTypeRtm(port_blob_num) || global.coinFuncs.blobTypeKcn(port_blob_num)) {
    ...
    this.convertAlgosToCoinPerf = function(algos_perf) {
        ...
        if      ("rx/0" in algos_perf)          coin_perf[""]     = coin_perf["ZEPH"] = coin_perf["SAL"] = algos_perf["rx/0"];
