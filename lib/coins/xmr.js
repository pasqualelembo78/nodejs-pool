    ...
    this.getPortAnyBlockHeaderByHash = function(port, blockHash, is_our_block, callback){
        ...
        if (port == 11898) {
            ...
            global.support.rpcPortDaemon(port, 'GetHeaderByHash', {"hash": Buffer.from(blockHash, 'hex').toJSON().data}, function (body) {
                ...
                body.result.header.reward = body.result.reward;
            ...
            global.support.rpcPortDaemon(port, 'GetTipInfo', null, function(body) {
                if (typeof(body) !== 'undefined' && body.hasOwnProperty('result')) {
                    global.coinFuncs.getPortBlockTemplate(port, function(bt) {
                        if (bt != null) {
                            body.result.metadata.height = body.result.metadata.best_block_height;
    ...
    this.getPortBlockTemplate = function(port, callback) {
		...
		method: 'getblocktemplate',
           ...
           global.support.rpcPortDaemon(port, 'GetNewBlockTemplate', { algo: {pow_algo: 0 } }, function(body){
                if (body && body.result) {
                   body.result.new_block_template.difficulty = body.result.miner_data.target_difficulty;
                   body.result.new_block_template.reward = body.result.miner_data.reward;
                       ...
                       if (body2 && body2.result) {
                           ...
                           return callback({ blocktemplate_blob: "00".repeat(3) + arr2hex(result.merge_mining_hash) + "00".repeat(8) + "02" + "00".repeat(32),
				             seed_hash: arr2hex(result.vm_key),
                                             ...
                                             difficulty: result.miner_data.target_difficulty,
                                             reward: result.miner_data.reward,
