...
function arr2hex(obj) {
  if (Array.isArray(obj)) {
    ...
    if (obj.every(item => typeof item === 'number')) {
      // Convert each number to 2-digit hex and join with commas
      return obj.map(n => n.toString(16).padStart(2, '0')).join('');
    } else {
      return obj.map(arr2hex); // recursively handle nested arrays
    }
  } else if (obj !== null && typeof obj === 'object') {
    const result = {};
    for (const [key, value] of Object.entries(obj)) {
      result[key] = arr2hex(value);
    }
    return result;
  }
  return obj;
...
function Coin(data){
    ...
    this.getPortBlockHeaderByID = function(port, blockId, callback){
        ...
        } else if (port == 18144 || port == 18146) { // XTM
            global.support.rpcPortDaemon(port, 'GetBlocks', {"heights": [blockId]}, function (body) {
                if (body && body.hasOwnProperty('result') && Array.isArray(body.result) && body.result.length === 1 && body.result[0].hasOwnProperty('block') ) {
                    return callback(null, arr2hex(body.result[0].block.header));
                } else {
                    console.error("getPortBlockHeaderByID(" + port + ", " + blockId + "): " + JSON.stringify(body));
                    return callback(true, body);
                }
    ...
    this.getPortAnyBlockHeaderByHash = function(port, blockHash, is_our_block, callback){
        ...
        if (port == 11898) {
            ...
            global.support.rpcPortDaemon2(port, '', { method: 'getblock', params: [ blockHash, 2 ] }, function (body) {
                ...
                if (port == 8766 || port == 19001 || port == 9766) { // XNA, RVN, CLORE
                  body.result.reward = 0;
                  for (const vout of body.result.tx[0].vout) {
                    if (vout.scriptPubKey.addresses && vout.scriptPubKey.addresses[0] === "AePr762UcuQrGoa3TRQpGMX6byRjuXw97A") continue; // ignore CLORE common address
                    body.result.reward += vout.value;
                  }
                } else { // body.result.reward = 3750; // BTRM, RTM
                  body.result.reward = 0;
                  for (const vout of body.result.tx[0].vout) {
                    if (vout.value > body.result.reward)
                      body.result.reward = vout.value;
                  }
                }
        ...
        } else if (port == 18144 || port == 18146) {
            global.support.rpcPortDaemon(port, 'GetHeaderByHash', {"hash": Buffer.from(blockHash, 'hex').toJSON().data}, function (body) {
                if ( typeof(body) === 'undefined' || !body.hasOwnProperty('result') ) {
                    console.error("getPortBlockHeaderByHash(" + port + ", " + blockHash + "): " + JSON.stringify(body));
                    return callback(true, body);
                }
                body.result.header.height = parseInt(body.result.header.height);
                body.result.header.reward = parseInt(body.result.reward);
                //body.result.header.difficulty = body.result.difficulty; // this is not target_difficulty
                return callback(null, arr2hex(body.result.header));
            });
    ...
    this.getPortLastBlockHeader = function(port, callback, no_error_report) {
		...
		return callback(null, { hash: bt.hash, timestamp: Date.now() / 1000, difficulty: bt.difficulty, height: bt.height, hash2: bt.hash2 });
            ...
            global.support.rpcPortDaemon(port, 'GetTipInfo', null, function(body) {
                if (typeof(body) !== 'undefined' && body.hasOwnProperty('result')) {
                  body.result.metadata.height = parseInt(body.result.metadata.best_block_height);
                  body.result.metadata.hash = body.result.metadata.best_block_hash;
                  return callback(null, arr2hex(body.result.metadata));
                } else {
                    if (!no_error_report) console.error("Last block header invalid: " + JSON.stringify(body));
                    return callback(true, body);
                }
	...
	global.coinFuncs.getPortLastBlockHeader(port, function (is_err, body) {
	    if (is_err) return callback(is_err, body);
            if (port == 18144 || port == 18146) {
                global.coinFuncs.getPortBlockTemplate(port, function(bt) {
                    if (bt != null) {
                        body.reward = bt.reward;
                        body.difficulty = bt.difficulty;
                        return callback(null, body);
                    } else {
                        if (!no_error_report) console.error("New XTM block template invalid");
                        return callback(true, body);
                    }
                });
	    } else return callback(is_err, body);
	}, no_error_report);
    };

    this.getPortLastBlockHeaderMM = function(port, callback, no_error_report) { // with merge mining header
        global.coinFuncs.getPortLastBlockHeader(port, function (is_err, body) {
            if (is_err) return callback(is_err, body);
            if (port == global.config.daemon.port) {
                global.coinFuncs.getPortLastBlockHeader(18144, function(is_err, body2) {
                    if (is_err) return callback(is_err, body);
                    body.mm = body2;
                    return callback(null, body);
                }, no_error_report);
    ...
    this.getPortBlockTemplate = function(port, callback) {
		...
		method: 'getblocktemplate',
                ...
                if (body && body.result) switch (parseInt(port)) {
                  ...
                  case 19001:
                  case 9766:
                    return callback(cnUtil.RavenBlockTemplate(body.result, global.config.pool["address_" + port.toString()]));
           ...
           global.support.rpcPortDaemon(port, 'GetNewBlockTemplate', { algo: {pow_algo: 0 } }, function(body){
                if (body && body.result) {
                   body.result.new_block_template.difficulty = parseInt(body.result.miner_data.target_difficulty);
                   body.result.new_block_template.reward = parseInt(body.result.miner_data.reward);
                   return callback(arr2hex(body.result.new_block_template));
                } else return callback(null);
            });

        } else if (port == 18146) {
           global.support.rpcPortDaemon(port, 'GetNewBlockTemplate', { algo: {pow_algo: 2 } }, function(body){
               if (body && body.result) {
                   global.support.rpcPortDaemon(port, 'GetNewBlockWithCoinbases', { new_template: body.result.new_block_template, coinbases: [{address: global.config.pool["address_" + port.toString()], value: 0, stealth_payment: true, revealed_value_proof: true, coinbase_extra: [] }] }, function(body2) {
                       if (body2 && body2.result) {
                           const result = body2.result;
                           // The format of blobtemplate_blob is:
                           ...
                           return callback({ blocktemplate_blob: "00".repeat(3) + arr2hex(result.merge_mining_hash) + "00".repeat(8) + "02" + "00".repeat(32),
				             seed_hash: arr2hex(result.vm_key),
                                             reserved_offset: 3+32+8+1,
                                             difficulty: parseInt(result.miner_data.target_difficulty),
                                             reward: parseInt(result.miner_data.reward),
				             height: parseInt(result.block.header.height),
                                             xtm_t_block: result.block,
                                           });
                       } else return global.coinFuncs.getPortBlockTemplate(port, function(bt2) { return callback(bt2); }); // try another time
                    });
                } else return callback(null);
            });
    ...
    this.blobTypeKcn   = function(blob_type_num) { return blob_type_num == 105; }

    this.blobTypeXTM_T = function(blob_type_num) { return blob_type_num == 106; }

    this.convertBlob = function(blobBuffer, port) {
        const blob_type_num = this.portBlobType(port, blobBuffer[0]);
        if (this.blobTypeDero(blob_type_num)) return Buffer.from(blobBuffer);
        if (this.blobTypeXTM_T(blob_type_num)) return Buffer.from(blobBuffer);
        let blob;
        try {
            ...
            } else if (this.blobTypeKcn(blob_type_num)) {
                blob = cnUtil.convertKcnBlob(blobBuffer);
            } else {
    ...
    this.constructNewBlob = function(blockTemplateBuffer, params, port) {
        ...
        if (global.coinFuncs.blobTypeGrin(blob_type_num)) {
            return cnUtil.construct_block_blob(blockTemplateBuffer,
            ...
            return cnUtil.constructNewDeroBlob(blockTemplateBuffer, Buffer.from(params.nonce, 'hex'));
        } else if (global.coinFuncs.blobTypeXTM_T(blob_type_num)) {
            let newBlob = Buffer.alloc(blockTemplateBuffer.length);
            blockTemplateBuffer.copy(newBlob);
            Buffer.from(params.nonce, 'hex').copy(newBlob, 3+32+4);
            return newBlob;
        } else if (global.coinFuncs.blobTypeRvn(blob_type_num)) {
            return cnUtil.constructNewRavenBlob(blockTemplateBuffer,
            ...
            return cnUtil.constructNewRtmBlob(blockTemplateBuffer, Buffer.from(params.nonce, 'hex'));
        } else if (global.coinFuncs.blobTypeKcn(blob_type_num)) {
            return cnUtil.constructNewKcnBlob(blockTemplateBuffer, Buffer.from(params.nonce, 'hex'));
        } else {
    ...
    this.getBlockID = function(blockBuffer, port){
        ...
        if (global.coinFuncs.blobTypeRtm(blob_type_num)) {
            ...
            return cnUtil.blockHashBuff3(cnUtil.convertKcnBlob(blockBuffer));
        } else {
            return cnUtil.get_block_id(blockBuffer, blob_type_num);
        }
    };

    this.getAuxChainXTM = function(obj) {
        if ( typeof (obj._aux) === 'object' &&
             obj._aux.chains instanceof Array && obj._aux.chains.length === 1 &&
             typeof (obj._aux.chains[0]) === 'object' && obj._aux.chains[0]
	...
	this.xtm_t_block        = template.xtm_t_block;
        ...
        if (global.coinFuncs.blobTypeRvn(port_blob_num) || global.coinFuncs.blobTypeRtm(port_blob_num) || global.coinFuncs.blobTypeKcn(port_blob_num) || global.coinFuncs.blobTypeXTM_T(port_blob_num) || is_dero) {
    ...
    this.convertAlgosToCoinPerf = function(algos_perf) {
        ...
        if      ("rx/0" in algos_perf)          coin_perf[""]     = coin_perf["ZEPH"] = coin_perf["SAL"] = coin_perf["XTM-T"] = algos_perf["rx/0"];
...
//        if      ("rx/wow" in algos_perf)        coin_perf["WOW"]  = algos_perf["rx/wow"];
        ...
        if      ("kawpow" in algos_perf)        coin_perf["RVN"]  = coin_perf["XNA"] = coin_perf["CLORE"] = algos_perf["kawpow"];
        if      ("kawpow4" in algos_perf)       coin_perf["XNA"] = coin_perf["CLORE"] = algos_perf["kawpow4"];
...
//        if      ("rx/graft" in algos_perf)      coin_perf["GRFT"] = algos_perf["rx/graft"];
    ...
    this.slowHashBuff = function(convertedBlob, blockTemplate, nonce, mixhash) {
	    ...
	    case 13007: return multiHashing.cryptonight_pico(convertedBlob, 0);					    	   // Iridium
            ...
            case 18146: return multiHashing.randomx(convertedBlob, Buffer.from(blockTemplate.seed_hash, 'hex'), 0);        // XTM-T
            case 17767: return multiHashing.randomx(convertedBlob, Buffer.from(blockTemplate.seed_hash, 'hex'), 0);	   // ZEPH
            case 19081: return multiHashing.randomx(convertedBlob, Buffer.from(blockTemplate.seed_hash, 'hex'), 0);	   // SAL
...
//            case 34568:
    ...
    this.blobTypeStr = function(port, version) {
	    ...
	    case 10225: return "raptoreum";       // BTRM
            ...
            case 17767: return "cryptonote_zeph"; // ZEPH
            case 19081: return "cryptonote_sal";  // SAL
            ...
            case 11812: return "cryptonote_xla";  // XLA
	    case 18146: return "xtm-t";           // XTM-T
