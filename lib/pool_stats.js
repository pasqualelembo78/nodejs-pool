...
function get_cmc_price(symbol, callback) {
    ...
    global.support.https_get("https://pro-api.coinmarketcap.com/v1/cryptocurrency/quotes/latest?slug=" + slug + "&convert=" + symbol + "&CMC_PRO_API_KEY=" + global.config.general.cmcKey, function (res) {
       if (res instanceof Object && "data" in res && "quote" in res.data[Object.keys(res.data)[0]] && symbol in res.data[Object.keys(res.data)[0]].quote) {
    ...
    async.series([
        ...
        function (callback) {
            if (typeof(poolType) === 'undefined' && price_btc == 0 && price_usd == 0 && price_eur == 0) {
                return get_cmc(function(prices) { return callback(null, prices); });
        ...
        setTimeout(updatePoolStats, 60*1000, poolType);
    });
}
...
function updateBlockHeader() {
    const ports = global.config.daemon.enableAlgoSwitching ? global.coinFuncs.getPORTS() : [ global.config.daemon.port ];
    async.eachSeries(ports, function(port, next) {
        global.coinFuncs.getPortLastBlockHeader(port, function(err, body){
            if (err) return next();
            switch (port) {
                case 18144:  if (body.pow.powAlgo === 1) return next(); break; // RandomXM
                //case 18146:  if (body.pow.powAlgo !== 2) next(); break; // RandomXT
            }
            network_info[port] = {
                difficulty: parseInt(body.difficulty),
                hash:       body.hash ? body.hash : body.hashrate,
                height:     body.height,
                value:      body.reward,
