...
function templateUpdate2(coin, port, isHashChange, coinHashFactor, isHashFactorChange, body_header, times_failed) {
    if (port == 8545 || port == 8645) {
        ...
        if (!body_bt) {
	    if (times_failed && times_failed >= 2) {
                console.error("Block template request failed for " + port + " port");
                coinHashFactorUpdate(coin, 0);
	    } else setTimeout(templateUpdate2, 500, coin, port, isHashChange, coinHashFactor, isHashFactorChange, body_header, times_failed ? times_failed + 1 : 1);
            return;
        }
...
function templateUpdate(coin, repeating, isForceBT) {
    ...
    if (coinHashFactor) {
      global.coinFuncs.getPortLastBlockHeader(port, function (err, body) {
        if (!newCoinHashFactor[coin]) {
            ...
            if ( isHashChange || isTimeChange || isForceBT === true) {
    ...
    let block_notify_server = net.createServer(function (socket) {
        ...
        socket.on('end', function () {
            ...
            if (typeof(coin) === 'undefined') {
                ...
                templateUpdate(coin, false, true);
            }
        });
