...
function templateUpdate2(coin, port, isHashChange, isMMHashChange, coinHashFactor, isHashFactorChange, body_header, times_failed) {
	...
	if (body_header.mm && aux_chain_xtm && aux_chain_xtm.height < body_header.mm.height) { // returned some not yet updated BT by XTM merge mining proxy
	    times_failed += 1;
      ...
      global.coinFuncs.getPortLastBlockHeaderMM(port, function (err, body) {
        if (!newCoinHashFactor[coin]) {
            ...
            if ( isHashChange || isMMHashChange || isTimeChange) {
                ...
                if (body.mm) lastBlockHashMM[coin] = body.mm.hash;
