...
function templateUpdate2(coin, port, isHashChange, isMMHashChange, coinHashFactor, isHashFactorChange, body_header, times_failed) {
    if (port == 8545 || port == 8645) {
        ...
        if (body_header.height < lastBlockHeight[coin]) {
          console.error("Ignore block template request attempt returned outdated template for " + port + " port (height " + body_header.height + " while " + lastBlockHeight[coin] + " height needed)");
          return;
        }
        ...
        if (body_header.mm && aux_chain_xtm) {
            if (body_header.mm.height < lastBlockHeightMM[coin]) {
                console.error("Ignore block template request attempt returned outdated template for " + port + " port (MM height " + body_header.mm.height + " while " + lastBlockHeightMM[coin] + " MM height needed)");
                return;
            }
        }
