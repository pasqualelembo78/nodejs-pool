...
function submit_block(miner, job, blockTemplate, blockData, resultBuff, hashDiff, isTrustedShare, isParentBlock, portUsedToSubmit, submit_blockCB) {
    ...
    let reply_fn = function (rpcResult, rpcStatus, port, submit_blockCB) {
        ...
        const submit_coin = is_tari_port ? "TXM" : blockTemplate.coin;
        ...
        // Success! Submitted a block without an issue.
            ...
            get_block_id(function(newBlockHash) {
                ...
                if (is_main_port && !is_tari_port) {
                    ...
                    if (!is_tari_port) global.database.storeAltBlock(Math.floor(time_now / 1000), global.protos.AltBlock.encode({
                        hash:          newBlockHash,
                        difficulty:    blockTemplate.difficulty,
                        shares:        0,
                        timestamp:     time_now,
            ...
            if (!portUsedToSubmit) {
                ...
                setTimeout(submit_block, 500, miner, job, blockTemplate, blockData, resultBuff, hashDiff, isTrustedShare, isParentBlock, port, submit_blockCB);
            } else {
    ...
    if (blockTemplate.port == 11898) {
        ...
        if (is_tari) { // no XMR
           ...
           if (!portUsedToSubmit || portUsedToSubmit === blockTemplate.port + 2)
           ...
           // Ignore XTM result and use XMR as main result here
           if (!portUsedToSubmit || portUsedToSubmit === blockTemplate.port)
...
function processShare(miner, job, blockTemplate, params, processShareCB) {
    ...
    verifyShare(function(hashDiff, resultBuff, blockData, isTrustedShare, isNeedCheckBlockDiff) {
        ...
        // TODO: Probalby needs to be integrated into submit_block as well for more uniform processing logic
