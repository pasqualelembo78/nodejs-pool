...
function templateUpdate2(coin, port, isHashChange, coinHashFactor, isHashFactorChange, body_header) {
    if (port == 8545 || port == 8645) {
        ...
        if (isHashChange || (isTimeChange && (!isRewardCheckReady || body_bt.expected_reward !== lastBlockReward[coin])) || isRewardChange) {
      ...
      global.coinFuncs.getPortLastBlockHeader(port, function (err, body) {
        if (!newCoinHashFactor[coin]) {
            ...
            if ( isHashChange || isTimeChange) {
                ...
                templateUpdate2(coin, port, isHashChange, coinHashFactor, isHashFactorChange, body);
...
function submit_block(miner, job, blockTemplate, blockData, resultBuff, hashDiff, isTrustedShare, isParentBlock, isRetrySubmitBlock, submit_blockCB) {
    const is_tari = global.config.daemon.port == blockTemplate.port && parseInt(hashDiff) < blockTemplate.wide_difficulty;
    const submit_coin = is_tari ? "TXM" : blockTemplate.coin;
    ...
    let reply_fn = function (rpcResult, rpcStatus) {
        ...
        if (rpcResult && (rpcResult.error || rpcResult.result === "high-hash" || rpcResult.result === "bad-txnmrklroot" || rpcResult.result === "bad-cbtx-mnmerkleroot")) { // did not manage to submit a block
            ...
            console.error(threadName + "Error submitting " + submit_coin + " (port " + blockTemplate.port + ") block at height " +
            ...
            if (isNotifyAdmin) setTimeout(function() { // only alert if block height is not changed in the nearest time
                global.coinFuncs.getPortLastBlockHeader(blockTemplate.port, function(err, body) {
                    ...
                    if (blockTemplate.height == body.height + 1) global.support.sendEmail(global.config.general.adminEmail,
                        "FYI: Can't submit " + submit_coin + " block to deamon on " + blockTemplate.port + " port",
                        "The pool server: " + global.config.hostname + " can't submit block to deamon on " + blockTemplate.port + " port\n" +
                        "Input: " + blockDataStr + "\n" +
                        threadName + "Error submitting " + submit_coin + " block at " + blockTemplate.height + " height from " + miner.logString +
        ...
        // Success! Submitted a block without an issue.
                          ...
                          ( typeof(rpcResult.result) !== 'undefined' && !(typeof(rpcResult.result) === 'object' && rpcResult.result && !rpcResult.result.block_id && rpcResult.result.status !== "OK") ) ||
            ...
            const get_block_id = function(cb) {
                ...
                // TARI
                } else if ( global.config.daemon.port == blockTemplate.port &&
                            typeof (rpcResult.result) === 'object' && rpcResult.result && !rpcResult.result.block_id && // no XMR block
                            typeof (rpcResult.result._aux) === 'object' &&
                            rpcResult.result._aux.chains instanceof Array &&
                            rpcResult.result._aux.chains.length === 1 &&
                            typeof (rpcResult.result._aux.chains[0]) === 'object' &&
                            typeof (rpcResult.result._aux.chains[0].block_hash) === 'string'
                          ) {
            ...
            get_block_id(function(newBlockHash) {
                const is_tari2 = global.config.daemon.port == blockTemplate.port && typeof (rpcResult.result) === 'object' && rpcResult.result && !rpcResult.result.block_id;
                console.log(threadName + "New " + submit_coin + " (port " + blockTemplate.port + ") block " + newBlockHash + " found at height " + blockTemplate.height + " by " + miner.logString +
                    ", isTrustedShare: " + isTrustedShare + " - submit result: " + JSON.stringify(rpcResult) +
                    ", block hex: \n" + blockDataStr
                );

                const time_now = Date.now();
                if (global.config.daemon.port == blockTemplate.port && !is_tari2) {
                    ...
                    if (!is_tari2) global.database.storeAltBlock(Math.floor(time_now / 1000), global.protos.AltBlock.encode({
                        hash:          newBlockHash,
                        difficulty:    blockTemplate.difficulty,
                        shares:        0,
                        timestamp:     time_now,
            ...
            if (isRetrySubmitBlock) {
                console.error(threadName + "Unknown error submitting " + submit_coin + " (port " + blockTemplate.port + ") block at height " +
                    blockTemplate.height + " (active block template height: " + activeBlockTemplates[blockTemplate.coin].height + ") from " +
                    miner.logString + ", isTrustedShare: " + isTrustedShare + ", rpcStatus: " + rpcStatus + ", error (" + (typeof rpcResult) + "): " + JSON.stringify(rpcResult) +
                    ", block hex: \n" + blockDataStr
                );
                setTimeout(submit_block, 500, miner, job, blockTemplate, blockData, resultBuff, hashDiff, isTrustedShare, isParentBlock, false, submit_blockCB);
            } else {
    ...
    if (blockTemplate.port == 11898) {
        ...
        global.support.rpcPortDaemon2(blockTemplate.port, "", { method: "eth_submitWork", params: blockData, jsonrpc: "2.0", id: 0 }, reply_fn);
    } else if (global.coinFuncs.blobTypeErg(job.blob_type_num)) {
        global.support.rpcPortDaemon2(blockTemplate.port, "mining/solution", {"n": blockData}, reply_fn);
...
function processShare(miner, job, blockTemplate, params, processShareCB) {
        ...
        if ( global.config.pool.trustedMiners &&
            ...
            if (isBlockDiffMatched) {
                if (miner.validShares || (miner.payout in minerWallets && minerWallets[miner.payout].hashes)) {
                    submit_block(miner, job, blockTemplate, blockData, resultBuff, hashDiff, true, true, true, function(block_submit_result) {
    ...
    verifyShare(function(hashDiff, resultBuff, blockData, isTrustedShare, isNeedCheckBlockDiff) {
        if (isNeedCheckBlockDiff && ge(hashDiff, blockTemplate.difficulty)) { // Submit block to the RPC Daemon.
            ...
            submit_block(miner, job, blockTemplate, blockData, resultBuff, hashDiff, isTrustedShare, true, true);
        }
        ...
        if (is_mm && ge(hashDiff, blockTemplate.child_template.difficulty)) { // Submit child block to the RPC Daemon.
            ...
            submit_block(miner, job, blockTemplate.child_template, shareBuffer2, resultBuff, hashDiff, isTrustedShare, false, true);
        }
