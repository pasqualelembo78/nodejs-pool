...
function updateCoinHashFactor(coin) {
    if (coin == "XTM") coinHashFactorUpdate(coin, 0);
...
function submit_block(miner, job, blockTemplate, blockData, resultBuff, hashDiff, isTrustedShare, isParentBlock, portUsedToSubmit, submit_blockCB) {
    ...
    let reply_fn = function (rpcResult, rpcStatus, port, submit_blockCB) {
        ...
        const report_height = is_tari_port ? blockTemplate.txm_height : blockTemplate.height;
        const active_height = is_tari_port ? activeBlockTemplates[blockTemplate.coin].txm_height : activeBlockTemplates[blockTemplate.coin].height;
    ...
    if (blockTemplate.port == 11898) {
        ...
        blockTemplate.xtm_t_block.header.nonce = blockData.readBigUInt32BE(3+32+4);
        blockTemplate.xtm_t_block.header.pow.pow_data = [...blockData.slice(3+32+8+1)];
	console.log("18146 " + blockTemplate.xtm_t_block.header.nonce.toString('hex'));
        console.log("18146 " + Buffer.from(blockTemplate.xtm_t_block.header.pow.pow_data).toString('hex'));
