...
let lastBlockHashMM      = {}; // coin key
let lastBlockHeightMM    = {}; // coin key
...
if (cluster.isMaster) {
    ...
    // reset last verified share counters every global.config.pool.minerThrottleShareWindow seconds
    setInterval(function () {
...
function addProxyMiner(miner) {
    ...
    if (!(proxyMinerName in proxyMiners)) {
        ...
        if (++ proxyMiners[proxyMinerName].count > global.config.pool.workerMax && !miner.xmrig_proxy) {
             console.error(threadName + "Starting to long ban  " + wallet + " miner address");
    ...
    if (port in global.coinFuncs.getMM_PORTS()) {
        ...
        if (child_coin in activeBlockTemplates) {
            ...
            template.parent_blocktemplate_blob = global.coinFuncs.constructMMParentBlockBlob(
                Buffer.from(rpc_template.blocktemplate_blob, 'hex'), port, template.child_template_buffer
            ).toString('hex');
        }
    }
...
function templateUpdate3(coin, port, coinHashFactor, isHashFactorChange, body_bt) {
...
function templateUpdate2(coin, port, isHashChange, isMMHashChange, coinHashFactor, isHashFactorChange, body_header, times_failed) {
    if (port == 8545 || port == 8645) {
        return templateUpdate3(coin, port, coinHashFactor, isHashFactorChange, body_header);
    } else global.coinFuncs.getPortBlockTemplate(port, function (body_bt) {
        ...
        if (!body_bt) {
            times_failed += 1;
            console.error("Block template request attempt " + times_failed + " failed for " + port + " port");
	    if (times_failed <= 2) {
		setTimeout(templateUpdate2, 500, coin, port, isHashChange, isMMHashChange, coinHashFactor, isHashFactorChange, body_header, times_failed);
            } else coinHashFactorUpdate(coin, 0);
            return;
        }

	const aux_chain_xtm = global.coinFuncs.getAuxChainXTM(body_bt);
	if (body_header.mm && aux_chain_xtm) {
	    if (aux_chain_xtm.height < body_header.mm.height) { // returned some not yet updated BT by XTM merge mining proxy
                times_failed += 1;
                console.error("Block template request attempt " + times_failed + " returned old template for " + port + " port (height " + aux_chain_xtm.height + " while " + body_header.mm.height + " height needed");
   	        if (times_failed <= 2) {
                    setTimeout(templateUpdate2, 500, coin, port, isHashChange, isMMHashChange, coinHashFactor, isHashFactorChange, body_header, times_failed);
  	        } else coinHashFactorUpdate(coin, 0);
		return;
            }
	    if (aux_chain_xtm.height < lastBlockHeightMM[coin]) {
                console.error("Ignore block template request attempt returned outdated template for " + port + " port (MM height " + aux_chain_xtm.height + " while " + lastBlockHeightMM[coin] + " MM height needed");
		return;
	    }
	}

        if (body_bt.height < lastBlockHeight[coin]) {
          console.error("Ignore block template request attempt returned outdated template for " + port + " port (height " + body_bt.height + " while " + lastBlockHeight[coin] + " height needed");
	  return;
        }
        ...
        const isRewardCheckReady = body_bt.expected_reward && (coin in lastBlockReward) && lastBlockReward[coin];
        const isRewardChange     = isRewardCheckReady && body_bt.expected_reward / lastBlockReward[coin] > 1.01;

        if (isHashChange || isMMHashChange || (isTimeChange && (!isRewardCheckReady || body_bt.expected_reward !== lastBlockReward[coin])) || isRewardChange) {
            lastBlockKeepTime[coin] = time_now;
            lastBlockReward[coin]   = body_bt.expected_reward;
            return templateUpdate3(coin, port, coinHashFactor, isHashFactorChange, body_bt);
        }
    });
}
...
function setNewBlockTemplate(template) {
	...
	const xtm_height     = aux_chain_xtm ? aux_chain_xtm.height : '';
	const xtm_difficulty = aux_chain_xtm ? aux_chain_xtm.difficulty : '';
        console.log('[*] New ' + coin_str + 'block to mine at ' + template.height + (xtm_height ? '/' + xtm_height : '') + ' height with ' + template.difficulty + (xtm_difficulty ? '/' + xtm_difficulty : '') + ' difficulty and ' + template.port + ' port (with coin hash factor ' + template.coinHashFactor + ")");
    } else {
        debug(threadName + 'New block to mine at ' + template.height + (template.xtm_height ? '/' + template.xtm_height : '') + ' height with ' + template.difficulty + ' difficulty and ' + template.port + ' port');
    }
...
// Print extra info for these wallets
let wallet_debug = {};
// Do not ban from these IPs
let ip_whitelist = {};
...
function Miner(id, login, pass, rigid, ipAddress, startingDiff, pushMessage, protoVersion, portType, port, agent, algos, algos_perf, algo_min_time) {
    ...
    for (let index = 1; index < login_div_split.length - 1; index += 2) {
        ...
        if (address in bannedTmpWallets) {
            this.error = "Temporary (10 minutes max) banned payment address " + address;
            this.valid_miner = false;
            return;
        }
        if (address in bannedBigTmpWallets) {
            this.error = "Temporary (one hour max) ban since you connected too many workers. Please use proxy (https://github.com/MoneroOcean/xmrig-proxy)";
            this.valid_miner = false;
    ...
    if (this.payout in bannedTmpWallets) {
        this.error = "Temporary (10 minutes max) banned payment address " + this.payout;
        this.valid_miner = false;
        return;
    }

    if (this.payout in bannedBigTmpWallets) {
        this.error = "Temporary (one hour max) ban since you connected too many workers. Please use proxy (https://github.com/MoneroOcean/xmrig-proxy)";
        this.valid_miner = false;
        this.delay_reply = 600;
        return;
	...
	const blob_type_num = global.coinFuncs.portBlobType(global.coinFuncs.COIN2PORT(this.curr_coin));
        if (    global.coinFuncs.blobTypeRvn(blob_type_num) ||
                global.coinFuncs.blobTypeEth(blob_type_num) ||
                global.coinFuncs.blobTypeErg(blob_type_num)
           ) {
    ...
    this.calcNewDiff = function () {
        ...
        if (proxyMiner && proxyMiner.hashes / (time_now - proxyMiner.connectTime) > this.difficulty) {
            ...
            history_time = 5;
            if (this.debugMiner) console.log(threadName + this.logString + " [WALLET DEBUG] calc proxy miner diff: " + miner.hashes + " / " + ((time_now - miner.connectTime) / 1000));
            ...
            min_diff = 10 * global.config.pool.minDifficulty;
            history_time = 5;
            if (this.debugMiner) console.log(threadName + this.logString + " [WALLET DEBUG] calc throttled miner diff: " + miner.hashes + " / " + ((time_now - miner.connectTime) / 1000));
        } else {
            miner = this;
            ...
            history_time = 60;
            if (this.debugMiner) console.log(threadName + this.logString + " [WALLET DEBUG] calc miner diff: " + miner.hashes + " / " + ((time_now - miner.connectTime) / 1000));
        }
    ...
    if (protoVersion === 1) {
        this.getCoinJob = function (coin, params) {
            ...
            const isErg = global.coinFuncs.blobTypeErg(blob_type_num);
            ...
            if (!this.proxy || isExtraNonceBT) {
                ...
                const isRvn  = global.coinFuncs.blobTypeRvn(blob_type_num);
                const newJob = {
                    id:             isRvn ? get_new_eth_job_id() : get_new_id(),
                    coin:           coin,
                    blob_type_num:  blob_type_num,
                    blockHash:      bt.idHash,
                    extraNonce:     isExtraNonceBT ? this.eth_extranonce : bt.extraNonce,
                    height:         bt.height,
		    ...
		    noncebytes: 4,
	            ...
	            newJob.id,
                    ...
                    blob_hex,
                    true,
                    coin_diff // this will be popped and used for separate mining.set_difficulty message
                ]; else if (isErg) this.cachedJob = [
                    newJob.id,
                    bt.height,
                    bt.hash,
                    "",
                    "",
                    2, // curl http://localhost:9053/info: parameters.blockVersion
                    baseDiff.div(coin_diff).toString(),
                    "",
                    true
                ]; else this.cachedJob = {
                    blob:       blob_hex,
...
// store wallet_key (address, paymentID, poolTypeEnum, port) -> worker_name -> isTrustedShare -> (height, difficulty, time, acc, acc2)
let walletAcc             = {};
...
function submit_block(miner, job, blockTemplate, blockData, resultBuff, hashDiff, isTrustedShare, isParentBlock, portUsedToSubmit, submit_blockCB) {
    ...
    let reply_fn = function (rpcResult, rpcStatus, port, submit_blockCB) {
        ...
        if (rpcResult && (rpcResult.error || rpcResult.result === "high-hash" || rpcResult.result === "bad-txnmrklroot" || rpcResult.result === "bad-cbtx-mnmerkleroot")) { // did not manage to submit a block
            ...
            console.error(threadName + "Error submitting " + report_coin + " (port " + report_port + ") block at height " +
        ...
        // Success! Submitted a block without an issue.
            ...
            const get_block_id = function(cb) {
                ...
                } else if ( is_tari_port && typeof (rpcResult.result) === 'object' && rpcResult.result &&
                            global.coinFuncs.getAuxChainXTM(rpcResult.result)
                          ) {
    ...
    if (blockTemplate.port == 11898) {
    ...
    } else if (global.coinFuncs.blobTypeRvn(job.blob_type_num) || global.coinFuncs.blobTypeRtm(job.blob_type_num) || global.coinFuncs.blobTypeKcn(job.blob_type_num)) {
        global.support.rpcPortDaemon2(blockTemplate.port, "", { method: "submitblock", params: [ blockData.toString('hex') ] }, std_reply_fn);
    } else if (global.coinFuncs.blobTypeEth(job.blob_type_num)) {
        global.support.rpcPortDaemon2(blockTemplate.port, "", { method: "eth_submitWork", params: blockData, jsonrpc: "2.0", id: 0 }, std_reply_fn);
    } else if (global.coinFuncs.blobTypeErg(job.blob_type_num)) {
        global.support.rpcPortDaemon2(blockTemplate.port, "mining/solution", {"n": blockData}, std_reply_fn);
    } else if (global.coinFuncs.blobTypeDero(job.blob_type_num)) {
        global.support.rpcPortDaemon(blockTemplate.port, "submitblock", [ blockTemplate.blocktemplate_blob, blockData.toString('hex') ], std_reply_fn);
    } else if (global.coinFuncs.blobTypeXTM_T(job.blob_type_num)) {
        blockTemplate.xtm_t_block.header.nonce = blockData.readUInt32BE(3+32+4).toString();
        blockTemplate.xtm_t_block.header.pow.pow_data = [...blockData.slice(3+32+8+1)];
        global.support.rpcPortDaemon(blockTemplate.port, "SubmitBlock", blockTemplate.xtm_t_block, std_reply_fn);
        ...
        if (is_xmr && (!portUsedToSubmit || portUsedToSubmit === blockTemplate.port + 2))
        ...
        if (is_xtm && (!portUsedToSubmit || portUsedToSubmit === blockTemplate.port))
...
function processShare(miner, job, blockTemplate, params, processShareCB) {
    ...
    let shareThrottled = function(processShareCB) {
	    ...
	    if (last_ver_shares > threshold) {
	        if (last_ver_shares == threshold) {
                    ...
                    console.error(threadName + "Throttled down miner share as invalid (diff " + job.rewarded_difficulty2 + ") submission from " + miner.logString);
                    invalid_share(miner);
    ...
    let verifyShare = function(verifyShareCB) {
        if (global.coinFuncs.blobTypeGrin(blob_type_num)) {
            ...
            if (shareThrottled(processShareCB)) return;
            const coinbaseBuffer = Buffer.concat([Buffer.from(blockTemplate.hash, 'hex'), Buffer.from(params.nonce, 'hex')]);
            const hashes = global.coinFuncs.slowHashBuff(coinbaseBuffer, blockTemplate);
...
function handleMinerData(socket, id, method, params, ip, portData, sendReply, sendReplyFinal, pushMessage) {
    switch (method) {
        case 'mining.authorize': // Eth/Raven/Erg only
        ...
        case 'mining.subscribe': { // Raven/Eth/Erg only
	        ...
	        sendReply(null, [ [ "mining.notify", get_new_id(), "EthereumStratum/1.0.0" ], eth_extranonce(new_id), 6 ]);
            } else {
                sendReplyFinal("Not enough extranoces. Switch to other pool node.");
            }
            break;
        ...
        case 'getjobtemplate': { // grin-mode miner only
            const minerId = socket.miner_id ? socket.miner_id : "";
            ...
            const nonce_sanity_check = function(blob_type_num, params) {
                if (global.coinFuncs.blobTypeGrin(blob_type_num)) {
                    ...
                    if (global.coinFuncs.nonceSize(blob_type_num) == 8) {
                        const isExtraNonceBT = global.coinFuncs.blobTypeEth(blob_type_num) ||
                                               global.coinFuncs.blobTypeErg(blob_type_num);
...
if (cluster.isMaster) {
    ...
    setInterval(function () {
        if ("" in activeBlockTemplates) {
            global.mysql.query("UPDATE pools SET last_checkin = ?, active = ?, blockIDTime = now(), blockID = ?, port = ? WHERE id = ?", [global.support.formatDate(Date.now()), true, activeBlockTemplates[""].height, activeBlockTemplates[""].port, global.config.pool_id]).catch(function (error) {
                console.error("SQL query failed: " + error);
            });
        } else {
            global.mysql.query("UPDATE pools SET last_checkin = ?, active = ? WHERE id = ?", [global.support.formatDate(Date.now()), true, global.config.pool_id]).catch(function (error) {
                console.error("SQL query failed: " + error);
            });
        }
        global.config.ports.forEach(function (portData) {
            ...
            global.mysql.query("UPDATE ports SET lastSeen = now(), miners = ? WHERE pool_id = ? AND network_port = ?", [miner_count, global.config.pool_id, portData.port]).catch(function (error) {
                console.error("SQL query failed: " + error);
            });
        });
    }, 30*1000);
