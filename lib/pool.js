...
function templateUpdate2(coin, port, isHashChange, isMMHashChange, coinHashFactor, isHashFactorChange, body_header, times_failed) {
    if (port == 8545 || port == 8645) {
        ...
        if (body_header.height < lastBlockHeight[coin]) {
          console.error("Ignore block template request attempt returned outdated template for " + port + " port (height " + (body_header.height+1) + " while " + (lastBlockHeight[coin]+1) + " height needed)");
          return;
        }

        const aux_chain_xtm = body_bt ? global.coinFuncs.getAuxChainXTM(body_bt) : null;
        if (body_header.mm && aux_chain_xtm) {
            if (body_header.mm.height < lastBlockHeightMM[coin]) {
                console.error("Ignore block template request attempt returned outdated template for " + port + " port (MM height " + (body_header.mm.height+1) + " while " + (lastBlockHeightMM[coin]+1) + " MM height needed)");
                return;
            }
        }
	...
	if (body_header.mm && aux_chain_xtm) {
	    if (aux_chain_xtm.height < body_header.mm.height+1) { // returned some not yet updated BT by XTM merge mining proxy
                times_failed += 1;
                console.error("Block template request attempt " + times_failed + " returned old template for " + port + " port (height " + aux_chain_xtm.height + " while " + (body_header.mm.height+1) + " height needed)");
   	        if (times_failed <= 2) {
                    setTimeout(templateUpdate2, 500, coin, port, isHashChange, isMMHashChange, coinHashFactor, isHashFactorChange, body_header, times_failed);
  	        } else coinHashFactorUpdate(coin, 0);
		return;
