    ...
    let reply_fn = function (rpcResult, rpcStatus) {
        ...
        // Success! Submitted a block without an issue.
            ...
            get_block_id(function(newBlockHash) {
                const is_tari = global.config.daemon.port == blockTemplate.port && typeof (rpcResult.result) === 'object' && rpcResult.result && !rpcResult.result.block_id;
                const mycoin = is_tari ? "TARI" : blockTemplate.coin;
                console.log(threadName + "New " + mycoin + " (port " + blockTemplate.port + ") block " + newBlockHash + " found at height " + blockTemplate.height + " by " + miner.logString +
                    ", isTrustedShare: " + isTrustedShare + " - submit result: " + JSON.stringify(rpcResult) +
                    ", block hex: \n" + blockDataStr
                );

                const time_now = Date.now();
                if (global.config.daemon.port == blockTemplate.port && !is_tari) {
                    ...
                    if (!is_tari) global.database.storeAltBlock(Math.floor(time_now / 1000), global.protos.AltBlock.encode({
                        hash:          newBlockHash,
                        difficulty:    blockTemplate.difficulty,
                        shares:        0,
                        timestamp:     time_now,
