...
function submit_block(miner, job, blockTemplate, blockData, resultBuff, hashDiff, isTrustedShare, isParentBlock, portUsedToSubmit, submit_blockCB) {
    ...
    if (blockTemplate.port == 11898) {
        ...
        const is_tari_only = parseInt(hashDiff) < blockTemplate.wide_difficulty;
        // We assume XMR daemon is on +2 port here
        if (!is_tari_only && (!portUsedToSubmit || portUsedToSubmit === blockTemplate.port + 2))
        ...
        if (!portUsedToSubmit || portUsedToSubmit === blockTemplate.port)
          global.support.rpcPortDaemon(blockTemplate.port, "submitblock", [ blockData.toString('hex') ], function (rpcResult, rpcStatus) {
            return reply_fn(rpcResult, rpcStatus, blockTemplate.port, is_tari_only ? submit_blockCB : null);
          });
    } else {
