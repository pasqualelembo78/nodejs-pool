        ...
        shareObject.forEach(function(share){
            ...
            if (typeof(share.raw_shares) === "number") {
                ...
                if (port_suffix === "") {
                    ...
                    port_suffix = "_18144"; // TXM special case
                }
                if (port_suffix !== "") {
    ...
    this.storeBlock = function(blockId, blockData, callback){
        try{
            ...
            global.coinFuncs.getBlockHeaderByHash(blockDataDecoded.hash, function(err, header){
                ...
                let txn = global.database.env.beginTxn();
                // this is very rare case if block with same height is found by several leafs (one that is stored later should be orphan with 0 reward)
    ...
    this.storeAltBlock = function(blockId, blockData, callback){
        try{
            ...
            global.coinFuncs.getPortBlockHeaderByHash(blockDataDecoded.port, blockDataDecoded.hash, function(err, header){
                ...
                if (blockDataDecoded.port == 20206 && header.depth < 30) {
                    console.log("Delaying " + blockDataDecoded.port + " port block hash " + blockDataDecoded.hash);
                    setTimeout(function () { return callback(false) }, 30*1000);
                    return;
                }
                if (err || typeof(header) === 'undefined' || !header || !header.reward) { // bad block and not orphan
                ...
                let txn = global.database.env.beginTxn();
                let blockData2;
                while ((blockData2 = txn.getBinary(global.database.altblockDB, blockId)) !== null) {
                    const blockDataDecoded2 = global.protos.AltBlock.decode(blockData2);
                    if (blockDataDecoded2.hash === blockDataDecoded.hash) {
                        txn.abort();
                        console.error("Can't store already stored altblock with " + blockDataDecoded.hash.toString() + " hash: " + JSON.stringify(blockDataDecoded));
                        return callback(true);
                    }
            ...
            function(oldestLockedBlockHeight, oldestLockedBlockDifficulty, callback){
                global.coinFuncs.getLastBlockHeader(function(err, body){
                    ...
                    if (oldestLockedBlockHeight === null){
                        ...
                        console.log("Block depth to keep is " + (body.height - oldestLockedBlockHeight));
